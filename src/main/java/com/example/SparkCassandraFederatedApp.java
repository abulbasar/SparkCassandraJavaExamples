...
public class SparkCassandraFederatedApp {

	private static SparkSession spark = null;

	public static Dataset<Row> dataFrameFromFileSystem(String path, String tableName) {
		Dataset<Row> dataset = spark.read().format("csv").option("header", true).option("inferSchema", true).load(path);
		dataset.createOrReplaceTempView(tableName);
		return dataset;
	}

	public static Dataset<Row> dataframeFromCassandraTable(String keySpace, String tableName, String viewName) {
	...
	public static void main(String[] args) throws IOException {
		...
		SparkConf conf = new SparkConf()
				.setAppName(SparkCassandraFederatedApp.class.getName())
		...
		System.out.println("Web UI" + spark.sparkContext().uiWebUrl());


		Dataset<Row> movies = dataframeFromCassandraTable("demo", "movies", "movies");
		Dataset<Row> ratings = dataFrameFromFileSystem(dataDir + "/ratings.csv", "ratings");
...
//		cassandraConnection.close();
		...
		dataframeFromCassandraTable("demo", "movies_agg", "movies_agg");
